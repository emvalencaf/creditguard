...
def get_smote_data(X_train: pd.DataFrame, 
    ...
    """
    ml_logging.info("Checking class distribution before applying SMOTE.")
    class_counts = y_train.value_counts()
    ml_logging.info(f"Class distribution before SMOTE:\n{class_counts}")
    
    y_train = y_train.values.ravel()

    if class_counts.min() / class_counts.max() >= 0.8:
        ml_logging.info("Dataset is already balanced. Skipping SMOTE.")
        return X_train, y_train

    try:
        ml_logging.info("Applying SMOTE to balance dataset.")
        ...
        ml_logging.info("SMOTE applied successfully.")
        
        return X_train_resampled, y_train_resampled

    except Exception as e:
        ml_logging.error(f"Error while applying SMOTE: {e}")
...
def get_training_data(features: pd.DataFrame,
    ...
    """
    ml_logging.info("Splitting dataset into training and testing sets.")
    target = target.round().astype(int)
    ml_logging.info(f'Features size: {features.shape}')
    ml_logging.info(f"Target size:{target.shape}")
    
    try:
        X_train, X_test, y_train, y_test = train_test_split(features, target,
                                                            test_size=0.2, random_state=42)
        ml_logging.info(f"Training set size: {X_train.shape[0]}, Test set size: {X_test.shape[0]}")
    except Exception as e:
        ml_logging.error(f"Error during train-test split: {e}")
        raise 
    ml_logging.info(f"X_train size:\n{X_train.shape}")
    ml_logging.info(f"Y_train size:\n{y_train.shape}")

    ml_logging.info(f"Features cols:\n{X_train.columns.tolist()}")
    ml_logging.info(f"Features cols datatype:\n{X_train.dtypes}")
